...
Future work should build on these findings by exploring more integrated and dynamic models:

1.  **Integrated Optimization:** Move towards models that simultaneously optimize the **order batching** and the **picker routing** problems, aiming for a true global optimum rather than sequential sub-optimization.
2.  **Advanced Metaheuristics and Constraints:** Extend powerful metaheuristics (e.g., using extensions of LKH) to incorporate complex real-world constraints such as **picker capacity**, **multiple unit load devices (UdM)**, and delivery **time windows**.
3.  **Real-Time Adaptive Routing:** Further investigate and develop models based on **Deep Reinforcement Learning (DRL)** for dynamic routing decisions, which could adapt instantaneously to real-time changes or arrivals of new high-priority orders in the system.
